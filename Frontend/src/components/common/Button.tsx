...
// ms
const TRIGGER_TTL = 750

export const Button = ({ display, hint, className, callback, keybind }: ButtonProps) => {
  ...
  return (
    <div className="relative w-fit p-1" onMouseLeave={() => setHovering(false)}>
      {hint ?
        <div className={`${showHint ? "opacity-100" : "opacity-0 select-none pointer-events-none"} absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-fit overflow-hidden transition-normal duration-200 bg-black/40 backdrop-blur-md py-1 px-2 text-sm rounded-md text-white font-medium whitespace-nowrap`}>
          <span>{hint}</span>
          {keybind ?
            <><br /><span className="text-xs font-light">({keybind})</span></>
            : null}
        </div>
        : null}
