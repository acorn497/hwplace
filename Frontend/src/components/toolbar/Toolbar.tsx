...
const ToolMap = [
  ...
  { tool: Tool.SETTING, icon: <Settings /> },
...
export const Toolbar = () => {
  ...
  return (
    <div className="fixed flex flex-col items-center bottom-10 left-1/2 -translate-x-1/2">
      <div className="relative flex items-center gap-2 bg-primary-modal backdrop-blur-md px-2 py-1.5 rounded-2xl border border-slate-900/10 shadow-sm shadow-black/5 mt-4">
          ...
          return (
            ...
            >
              <div className={`w-5 h-5 flex items-center justify-center transition-transform duration-200 ${isActive ? 'scale-105' : 'scale-100'}`}>
                {tool.icon}
                <span className={`absolute -right-1.5 -bottom-2 text-xs ${isActive ? 'text-white' : 'text-slate-400'} transition-colors duration-200`}>{index + 1}</span>
              </div>
            </button>
          );
        })}
