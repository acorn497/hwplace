...
const Notification = forwardRef<NotificationHandle, NotificationProps>(
  ({ title, content, method = types.INFO, callback, duration = 20 }, ref) => {
    ...
    useEffect(() => {
      if (timer > duration) setFadeOut(true);
    }, [timer, duration]);
